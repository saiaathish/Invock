...
* Current tests executed for this revision: `pnpm test` passed 283/283; `pnpm typecheck` and `pnpm build` passed; the bounded property, fuzz, chaos, security, authority, transport, and API suites passed. These results are local evidence, not proof of every production or external-deployment vector in the calculus above.
