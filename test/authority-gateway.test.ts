...
`));
    ...
    const gate = new InvocationGate(policy, new StaticDescriptorRegistry({ read: descriptor }), store, { cwd: directory, projectRoot: directory, organizationDomains: [], sessionId: "authority-session", principal: { principalId: "test", clientId: "test", scopes: [] }, now: () => now });
