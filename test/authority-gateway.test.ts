...
`));
    ...
    const gate = new InvocationGate(policy, new StaticDescriptorRegistry({ read: descriptor }), store, { cwd: directory, projectRoot: directory, organizationDomains: [], sessionId: "authority-session", principal: { principalId: "test", agentId: "worker", clientId: "test", scopes: [] }, now: () => now }, { allowUnboundForTests: true, requireContainment: false });
...
`));
    const descriptor = { fields: [{ pointer: "/path", type: "path" as const, access: "read" as const }], inputSchema: { type: "object", properties: { path: { type: "string" } }, required: ["path"], additionalProperties: false } };
    const gate = new InvocationGate(policy, new StaticDescriptorRegistry({ read: descriptor }), store, { cwd: directory, projectRoot: directory, organizationDomains: [], sessionId: "session-b", principal: { principalId: "test", clientId: "test", scopes: [] } }, { allowUnboundForTests: true });
    const now = new Date("2027-01-01T00:00:00.000Z");
    const target = join(directory, "safe.txt");
    ...
    if (outcome.kind === "respond") assert.match(String(outcome.response.result.structuredContent?.reasonCodes), /NORMALIZATION_FAILED|EFFECTIVE_AUTHORITY_DENIED|IDENTITY_BINDING_INVALID/);
