...
test("root lease issuer is bound to the capsule provenance", () => {
  const now = new Date("2027-01-01T00:00:00Z");
  const capsule = activateIntentCapsule(createIntentCapsule(input({ rootIssuer: "control-plane" }), now), now);
  assert.throws(() => issueCapabilityLease(leaseInput(), capsule, undefined, now), /ROOT_LEASE_ISSUER_MISMATCH/u);
  const lease = issueCapabilityLease(leaseInput({ issuer: "control-plane" }), capsule, undefined, now);
  assert.equal(evaluateMonotonicAuthority(capsule, [lease], { tool: "read", capabilities: ["fs.read"], effects: ["data.observe"] }, now).allowed, true);
});
