...
export class InvockStore {
  ...
  complete(envelope: ActionEnvelope, decision: PolicyDecision, forwarded: boolean, result: unknown, approvalId?: string, now = new Date()): SignedReceipt {
    this.db.exec("BEGIN IMMEDIATE");
    let committed = false;
    try {
      ...
      const receipt = signReceipt(makeReceiptPayload({ instanceId: this.instanceId, sequence, envelope, decision, upstreamForwarded: forwarded, previousReceiptHash: previous?.receipt_hash ?? null, ...(forwarded ? { upstreamResultDigest: sha256(JSON.stringify(result)) } : {}), ...(approvalId ? { approvalId } : {}), now }), this.signing);
