...
export interface ForwardedCall { kind: "forward"; request: ToolCallRequest; envelope: ActionEnvelope; decision: PolicyDecision; approvalId?: string; }
...
export interface InvocationRuntimeOverrides { sessionId?: string; protocolEra?: string; }
...
export class InvocationGate {
  ...
  async authorizeInvocation(request: ToolCallRequest, overrides: InvocationRuntimeOverrides = {}): Promise<GateOutcome> {
    ...
    if (decision.verdict === "APPROVAL_REQUIRED" && request.id === undefined) {
      ...
      const receipt = this.store.complete(envelope, blocked, false, { blocked: true, notification: true }, undefined, now);
    ...
    if (decision.verdict === "ALLOW") {
      ...
      return { kind: "forward", request: authorizedRequest, envelope, decision };
    }
    if (decision.verdict === "BLOCK") {
      this.store.recordInterception(envelope, decision, now); const receipt = this.store.complete(envelope, decision, false, { blocked: true }, undefined, now);
    ...
    if (typeof suppliedApproval === "string") {
      ...
      if (this.store.consumeApproval(suppliedApproval, envelope, decision, allowed, now)) {
        const canonicalArguments = JSON.parse(canonicalize(request.params.arguments ?? {})) as Record<string, unknown>;
        return { kind: "forward", request: { ...request, params: { name: request.params.name, arguments: canonicalArguments, _meta: { "io.invock/approval-id": suppliedApproval } } }, envelope, decision: allowed, approvalId: suppliedApproval };
      }
      const denied = blockedDecision(decision, "APPROVAL_INVALID_OR_CONSUMED"); this.store.recordInterception(envelope, denied, now); const receipt = this.store.complete(envelope, denied, false, { approval: "invalid" }, suppliedApproval, now);
    ...
    const receipt = this.store.complete(envelope, decision, false, { approval: "required" }, approval.approvalId, now);
    if (request.id === undefined) return { kind: "notification", decision, receiptId: receipt.payload.receiptId };
  ...
  finish(forwarded: ForwardedCall, result: ToolResult, now = this.contextBase.now?.() ?? new Date()): string {
    ...
    return this.store.complete(forwarded.envelope, forwarded.decision, true, result, forwarded.approvalId, now).payload.receiptId;
  }
  finishNotification(forwarded: ForwardedCall, now = this.contextBase.now?.() ?? new Date()): string { return this.store.complete(forwarded.envelope, forwarded.decision, true, { notification: true }, forwarded.approvalId, now).payload.receiptId; }
  fail(forwarded: ForwardedCall, message: string, now = this.contextBase.now?.() ?? new Date()): string { return this.store.complete(forwarded.envelope, forwarded.decision, true, { upstreamError: message }, forwarded.approvalId, now).payload.receiptId; }
}
