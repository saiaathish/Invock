...
export interface HttpMcpGatewayOptions { host?: string; port?: number; token?: string; candidate2026?: boolean; allowedOrigins?: string[]; requestTimeoutMs?: number; forward(message: JsonRpcMessage, signal: AbortSignal): Promise<JsonRpcMessage>; }
...
export async function startStreamableHttpGateway(gate: InvocationGate, options: HttpMcpGatewayOptions): Promise<HttpMcpGateway> {
  ...
  const server = createServer((request, response) => { void (async () => {
    ...
    if (request.method === "GET") { send(response, 405); return; } // independent SSE channels need a session/event router and are deliberately not faked.
    ...
    if (!isToolCall(message)) {
      ...
      try {
        const upstream = await options.forward(message, controller.signal);
        ...
        send(response, 200, upstream as JsonRpcResponse); return;
    ...
    if (outcome.kind === "respond") { if (requestId !== undefined) inFlight.delete(requestId); if (message.id === undefined) { send(response, 204); return; } send(response, 200, deniedToolCall(message.id, outcome.response.result)); return; }
    if (outcome.kind === "notification") {
      if (outcome.request) await options.forward(outcome.request, new AbortController().signal);
    ...
    try {
      const upstream = await options.forward(outcome.request, controller.signal);
      ...
      if (outcome.request.id === undefined) send(response, 202); else send(response, 200, { jsonrpc: "2.0", id: outcome.request.id, result });
  ...
  return await new Promise((resolve, reject) => { server.once("error", reject); server.listen(options.port ?? 0, host, () => { const address = server.address(); if (!address || typeof address === "string") { reject(new Error("Could not bind MCP HTTP gateway")); return; } resolve({ server, url: `http://${host}:${address.port}/mcp`, token, close: () => new Promise((done, fail) => server.close(error => error ? fail(error) : done())) }); }); });
}